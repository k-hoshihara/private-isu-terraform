...
description: >-
  ...
  there, stop for user review, then merge into that feature branch (not main)
  and git push. Use when a discrete implementation task starts, the user asks
...
All edits, commits, and tests happen in the child checkout.
...
Commit in the child when the task is a coherent unit.
...
- how to inspect (`git log`, `git diff <integration-branch>...HEAD` from the child)

Do not merge or push in this step.

## 5. After explicit approval only

Run these in the **parent** worktree, after the user says the child is OK:
...
Git fallback for the last step: `git worktree remove <child-path>`.

Merge target is the parent feature branch. Never merge to `main` / `master` unless the user named that branch.

If the merge conflicts, stop in the parent and report. Do not force-push.
