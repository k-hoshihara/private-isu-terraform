...
00200 のやり直しではない。Python 切替済み、ツールは [00200-setup.md](00200-setup.md) を起点にする。

前提:

- EC2 は 1 台。ベンチも同じホストから `http://localhost`
- 実装は [webapp-setup/python.md](webapp-setup/python.md)（`isu-python.service`）
- サイクルの見方は [00300-measure.md](00300-measure.md)
- 終盤のログ停止は [00400-ops.md](00400-ops.md#ログを止める終盤)。ここでは複製しない
...
導入そのものは [00200-setup.md](00200-setup.md#計測ツール)。入っていなければそこのブロックを打つ。ここでは確認と、このドリルでの使い方。
...
ptrace は [00200-setup.md](00200-setup.md#権限py-spy--ulimit) と `etc/sysctl/99-isucon-ptrace.conf`。取れないときは [6.](#6-トラブルシュート)。
...
永続化の drop-in は [00200-setup.md](00200-setup.md#ログnginx-ltsv--mysql-slow)。ドリルではまず今の値を見て、**最初から 0 で放置しない**。
...
見方は [00300-measure.md](00300-measure.md#インデックス): `type` は `ref` / `range` が欲しい。`ALL` はフルスキャン。`key` が `NULL` なら使っていない。

### alp / slp（導入は 00200）

マッチャは `etc/alp/matching_groups.json`。[00300-measure.md](00300-measure.md#alp) と同じ。
...
**1 ノブずつ。** 戻してから次へ。まとめて `my.cnf` を有効化しない。`innodb_buffer_pool_size` を最初のノブにしない（効かない・逆効果の報告がある。[00400-ops.md](00400-ops.md#nginx--mysql-の設定) と `etc/mysql/mysqld-isucon.cnf`）。
...
| nginx `access_log` を止める | 同じ |  |  | alp が空。手順は [00400](00400-ops.md#ログを止める終盤) |
...
ルールは [00300-measure.md](00300-measure.md#インデックス): **1 本ずつ**。`examined >> sent` を優先。LLM に投げるなら [prompts/index-from-slp.md](prompts/index-from-slp.md)。
...
再起動で初期化 SQL が DB を作り直す回は、終盤にもう一度 `SHOW INDEX`（[00400-ops.md](00400-ops.md#終盤チェック1700-以降)）。
...
DB がまだ mysqld で飽和しているうちは alp / slp を先に見る（[00300-measure.md](00300-measure.md#1-サイクル)）。抜けてからアプリ。
...
終盤に py-spy を残さない（[00400-ops.md](00400-ops.md#ログを止める終盤)）。
...
4. 終盤ならログ自体を止める → [00400-ops.md](00400-ops.md#ログを止める終盤)
...
- Ruby が enabled に戻っている → [webapp-setup/python.md](webapp-setup/python.md)
- 依存 → `sudo su - isucon` して `cd /home/isucon/private_isu/webapp/python && uv sync` から restart
- mysqld が 99-isucon*.cnf の直後に死んでいる → その drop-in を消して `systemctl restart mysql`（[00200-setup.md](00200-setup.md#mysqld-が起きないslow-設定の直後)）
