...
state はリモートで管理しておらず、CloudShell のホームディレクトリにのみ存在します。  
当該リージョンで最終セッションから120日間アクセスがないと、永続ストレージごと削除されます。

state を失うと、作成済みのリソースとの対応が分からなくなります。  
`terraform destroy` が使えなくなり、EC2 や VPC がコンソールから手作業で消すまで残り続けます。

そのため、中断する期間で判断します。

| 期間 | 対応 |
| --- | --- |
| 数日から数週間 | インスタンスを停止する。EBS の料金のみ発生します |
| 1ヶ月以上 | `terraform destroy` で削除し、state に何も残っていない状態にします |
